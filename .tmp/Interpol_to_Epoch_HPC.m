...
if stats.processed_subjects > 0
    ...
    % Save the summary to a .mat file
    summary_filename = sprintf('Processing_Summary_%s.mat', datestr(now, 'yyyy-mm-dd_HH-MM-SS'));
    summary_filepath = fullfile(epoched_dir, summary_filename);
    
    try
        save(summary_filepath, 'processing_summary', 'stats');
        fprintf('Statistics summary exported to: %s\n', summary_filepath);
        fprintf('Contains variables: processing_summary (structured format) and stats (raw format)\n');
    catch ME
