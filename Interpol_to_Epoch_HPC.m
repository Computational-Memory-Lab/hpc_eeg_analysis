%% EEG DATA EPOCHING AND PROCESSING SCRIPT - VERSION 3 (Robust Workflow)
% This script processes EEG data files with condition-specific trial removal
% using a robust, standard EEGLAB workflow that avoids data corruption.
%
% WORKFLOW:
...
%   EPOCH_WINDOW="-0.1,2.5" (or "-0.1 2.5")
%   VOLTAGE_DIFF=50
%   VOLTAGE_ABS=1000
epoch_window = parse_epoch_window_env(getenv('EPOCH_WINDOW'));
if isempty(epoch_window)
    epoch_window = [-0.1 1.5];
end
voltage_diff_threshold = parse_scalar_env(getenv('VOLTAGE_DIFF'), 50);
voltage_abs_threshold = parse_scalar_env(getenv('VOLTAGE_ABS'), 1000);
...
for file_idx = 1:length(file_info)
    ...
    EEG = pop_epoch(EEG, all_triggers, epoch_window, 'newname', sprintf('Subject %d (%s) - all epoched', s, study_id), 'epochinfo', 'yes');
    ...
    % Define the 6 conditions: {trial_type_name, trigger_string, accuracy_value}
    condition_def = { ...
        'Study_hits',         '11', 1; ...
        'Study_misses',       '11', 0;
        'Test_hits',          '21', 1;
    ...
    % NEW: Count original trials per condition for this subject
    ...
    % Track which epochs have been assigned to which condition
    epoch_condition_map = cell(1, EEG.trials);

    for i = 1:length(EEG.event)
        % Ensure event type is a string for comparison
        ...
        for c = 1:size(condition_def, 1)
            ...
            if isfield(EEG.event(i), 'Acc') && strcmp(event_type_str, cond_trigger) && EEG.event(i).Acc == cond_acc
                ...
                % Only count this condition once per epoch
                ...
                break; % Move to the next event once a match is found
            end
        end
    end
    ...
    % Define the 3 artifact rejection groups
    group_def = {'SME',           {'11'};
                 'Test_Intact',   {'21'};
                 'Test_Recombined', {'22'}};
...
function trial_indices_by_group = assign_epochs_to_groups_fixed(EEG)
    % Helper function to assign epochs to groups based on triggering event
    % This is in a separate function to help MATLAB's parfor analyzer
    ...
    % Extract event info as simple arrays
    ...
    % For each epoch, find triggering event (earliest with code 11/21/22)
...
function out = parse_scalar_env(raw_value, default_value)
out = default_value;
if isempty(raw_value)
    return;
end
parsed = str2double(strtrim(raw_value));
if isfinite(parsed)
    out = parsed;
else
    warning('Ignoring invalid numeric override "%s"; using default %.3f.', raw_value, default_value);
end
end

function out = parse_epoch_window_env(raw_value)
out = [];
if isempty(raw_value)
    return;
end

text = strtrim(raw_value);
if isempty(text)
    return;
end
...
if numel(parts) ~= 2
    warning('Ignoring invalid EPOCH_WINDOW override "%s"; expected two values.', raw_value);
    return;
end

vals = [str2double(strtrim(parts{1})), str2double(strtrim(parts{2}))];
if any(~isfinite(vals)) || vals(1) >= vals(2)
    warning('Ignoring invalid EPOCH_WINDOW override "%s"; expected start<end finite values.', raw_value);
    return;
end
